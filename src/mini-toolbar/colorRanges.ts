...
export const createColorExtension = (
  ...
  const colorField = StateField.define<ColorState>({
    create(state) {
      ...
      const decorations = isEnabled()
        ? buildDecorations(state, text, bg, underline)
        : Decoration.none;
    ...
    update(value, tr) {
      ...
      const decorations = isEnabled() && isMainView
